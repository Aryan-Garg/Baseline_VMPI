...
class LightFieldDataset(Dataset):
    ...
    def __getitem__(self, idx):
        file_paths = self.filenames[idx][:-1]
        prev_path, curr_path = file_paths.split(',')
        lf = np.load(os.path.join(self.lf_path, curr_path))/255.0
        prev_lf = np.load(os.path.join(self.lf_path, prev_path))/255.0
        ...
        prev_lf = torch.tensor(prev_lf, dtype=torch.float)
        if lf.shape[4] == 3:
            lf = lf.permute([0, 1, 4, 2, 3])
            prev_lf = prev_lf.permute([0, 1, 4, 2, 3])
        else:
            lf = lf.permute([1, 0, 2, 3, 4])
            prev_lf = prev_lf.permute([1, 0, 2, 3, 4])

        V, V, C, H, W = lf.shape
        lf = lf.reshape((V*V, C, H, W))
        prev_lf = prev_lf.reshape((V*V, C, H, W))
        lf = F.interpolate(lf, size=lfsize[:2], mode='nearest')
        prev_lf = F.interpolate(prev_lf, size=lfsize[:2], mode='nearest')
        H, W = lfsize[:2]
        lf = lf.reshape(V, V, C, H, W)
        prev_lf = prev_lf.reshape(V, V, C, H, W)
        ...
        # get deeplens depth
        file_path = curr_path.replace('npy', 'png')
