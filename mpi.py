...
def depth_rendering_one(central, disp, v, u, device=torch.device('cuda:0')):
    b_sz = central.size()[0]
    y_sz = central.size()[1]
    x_sz = central.size()[2]
    ...
    coord = coord.repeat([b_sz, 1, 1, 1]).to(device)

    #print(central.device, coord.device, device)
    target = torch.nn.functional.grid_sample(central.permute([0, 3, 1, 2]), coord)
    return target


def mpi_lf_wrapping(rgba, planes, v, u, device=torch.device('cuda:0')):
    ...
    for i in np.arange(len(planes)):
        outputs.append(depth_rendering_one(rgba[:,:,:,i,:], planes[i], v, u, device))
...
def mpi_lf_rendering(rgba, planes, v, u, device=torch.device('cuda:0')):
    #print(device)
    target_rgba = mpi_lf_wrapping(rgba, planes, v, u, device)
